...
def spatial_train_test_split_simple(coordinates, test_size=0.2, val_size=0.1, random_state=42):
    ...
    n_clusters = n_test_clusters + n_val_clusters + \
    ...
    print(
        f"  Train: {len(train_idx)} samples ({len(train_clusters)} spatial blocks)")
    print(f"  Val: {len(val_idx)} samples ({len(val_clusters)} spatial blocks)")
    print(
        f"  Test: {len(test_idx)} samples ({len(test_clusters)} spatial blocks)")
...
def prepare_inventory_only_features(df):
    ...
    for col in numerical_cols:
        try:
            ...
            print(
                f"    Warning: {col} appears non-numeric, treating as categorical")
...
def plot_results(train_history, threshold_results, feature_importance, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'xgboost_baseline_results.png',
...
def main():
    ...
    X, y, coordinates, label_encoders, feature_names = prepare_inventory_only_features(
        merged_df)
    ...
    print(f"\nClass distribution:")
    print(
        f"  Train: {y_train.sum()} yew / {len(y_train)-y_train.sum()} no-yew ({100*y_train.mean():.2f}%)")
    print(
        f"  Val: {y_val.sum()} yew / {len(y_val)-y_val.sum()} no-yew ({100*y_val.mean():.2f}%)")
    print(
        f"  Test: {y_test.sum()} yew / {len(y_test)-y_test.sum()} no-yew ({100*y_test.mean():.2f}%)")
    ...
    prec_default = precision_score(
        y_test, y_test_pred_default, zero_division=0)
    ...
    importance_df.to_csv(
        'results/tables/xgboost_feature_importance.csv', index=False)
    print("  Feature importance saved to results/tables/xgboost_feature_importance.csv")

    # Create visualizations
    Path('results/figures').mkdir(parents=True, exist_ok=True)
    plot_results(train_history, threshold_results,
                 feature_importance, Path('results/figures'))
