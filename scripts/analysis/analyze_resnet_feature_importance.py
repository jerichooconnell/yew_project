...
def permutation_importance(model, test_loader, device, feature_names, n_repeats=5):
    ...
    baseline_f1 = f1_score(
        targets_np, (baseline_preds > 0.5).astype(int), zero_division=0)
    ...
    for feat_idx, feat_name in enumerate(tqdm(feature_names, desc="Features")):
        ...
        for _ in range(n_repeats):
            ...
            permuted_f1 = f1_score(
                targets_np, (permuted_preds > 0.5).astype(int), zero_division=0)
...
def plot_importance_comparison(perm_importance, grad_importance, correlations, output_path):
    ...
    df_combined['Average'] = df_combined[[
        'Permutation', 'Gradient', 'Correlation']].mean(axis=1)
    df_combined = df_combined.sort_values('Average')

    x = np.arange(len(features))
    width = 0.25
    ax.barh(x - width, df_combined['Permutation'],
            width, label='Permutation', alpha=0.7)
    ax.barh(x, df_combined['Gradient'], width, label='Gradient', alpha=0.7)
    ax.barh(x + width, df_combined['Correlation'],
            width, label='Correlation', alpha=0.7)
    ax.set_yticks(x)
...
def main():
    ...
    results_df.to_csv(
        'results/tables/resnet_feature_importance.csv', index=False)
