...
def load_no_yew_sites(n_sites=100):
    ...
    if len(no_yew_sites) > n_sites:
        no_yew_sites = no_yew_sites.sample(n=n_sites, random_state=42)
...
def extract_single_site(site_data, output_dir):
    ...
    try:
        ...
        sample = composite_reprojected.sampleRectangle(region=region, defaultValue=0)
        ...
        if blue.shape != (patch_size, patch_size):
            ...
            blue = zoom(blue, (zoom_factor_y, zoom_factor_x), order=1)[:patch_size, :patch_size]
            green = zoom(green, (zoom_factor_y, zoom_factor_x), order=1)[:patch_size, :patch_size]
            red = zoom(red, (zoom_factor_y, zoom_factor_x), order=1)[:patch_size, :patch_size]
            nir = zoom(nir, (zoom_factor_y, zoom_factor_x), order=1)[:patch_size, :patch_size]
        
        # Save image
        image_array = np.stack([blue, green, red, nir], axis=0).astype(np.float32)
...
def main():
    ...
    no_yew_sites = load_no_yew_sites(n_sites=100)
    if len(no_yew_sites) == 0:
        print("\n✗ No non-yew sites found!")
        return
    
    output_dir = Path('data/ee_imagery/image_patches_64x64')
    extract_parallel(no_yew_sites, output_dir, max_workers=4)
