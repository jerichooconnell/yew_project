#!/usr/bin/env python3
"""
Ultra-Fast Yew Image Extraction (Parallel Processing)
=====================================================

Uses concurrent processing to extract yew site images in parallel.
...
def load_yew_sites_only():
    ...
    print(f"  Found {len(yew_sites)} yew sites")
...
def extract_single_site(site_data, output_dir):
    ...
    try:
        ...
        # Most measurements are pre-2015, but environmental conditions are stable
        ...
        # This ensures we get pixel-level data, not aggregated values
        ...
        image_path = output_dir / 'yew' / f"{site_id}.npy"
...
def extract_parallel(sites_df, output_dir, max_workers=4):
    ...
    print(
        f"\nExtracting {len(site_data_list)} yew sites with {max_workers} workers...")
...
def main():
    print("="*80)
    print("PARALLEL YEW SITE IMAGE EXTRACTION")
    print("="*80)
    ...
    extract_parallel(yew_sites, output_dir, max_workers=4)
