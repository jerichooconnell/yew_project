...
To translate the observed size structure into a viability statement, we seeded the Busing & Spies (1995) stage-projection matrix with the observed (FAIB + field) stage structure and projected it forward under contrasting scenarios, including a stochastic quasi-extinction analysis (Figure S5; `scripts/analysis/yew_pva.py`). Vital rates in the Busing matrix are calibrated to reproduce λ = 1.02 and q = 1.5 for undisturbed old-growth. We independently verified stage-specific annual survival using the 194 multi-visit FAIB TW trees (292 transitions, mean interval 13.1 yr); the FAIB-fitted mean annual survival (0.994/yr) is slightly lower than the Busing-calibrated value (0.998/yr) and does not alter the qualitative conclusions. FAIB-fitted growth-class advancement rates are slower than the Busing model, likely reflecting that the FAIB sample spans degraded as well as prime old-growth stands; Busing-calibrated growth rates are retained for projections (full comparison in `results/analysis/faib_vital_rates.json`). The baseline annual burn probability used in fire scenarios was updated from a placeholder (0.002/yr) to the BC fire-history estimate of 0.000794/yr (96,543 ha burned across the 980,000 ha study area over 1900–2024); this lower rate reinforces, rather than changes, the conclusion that historical fire rates alone are insufficient to drive population-level decline without concurrent logging. In intact, protected old-growth the modelled stand is viable (λ = 1.02, quasi-extinction probability ≈ 0 over 200 yr), consistent with Busing & Spies' conclusion that undisturbed stands are stable to slowly growing. Critically, the matrix elasticities are highest for recruitment and growth through the small and mid-size classes (stages 4–5: 0.30, 0.28) and lowest for large-tree survival (stages 6–7: 0.06, 0.02), identifying recruitment, not adult survival, as the demographic lever for stand-level viability. The two best-documented stressors act on exactly this lever. Ungulate browse — which removes 60–80% of seedlings and saplings in high-density areas (§2.6.4) — drops the modelled growth rate below replacement on its own (λ = 0.99, quasi-extinction probability 12%); combined with continued attrition of the large, bark-rich seed-producers (whose reproductive value, 8.5 and 8.2, is the highest of any stage), it drives quasi-extinction probability to 42% over 200 yr. By contrast, large-tree removal *alone* barely changes λ (1.02), because yew's demography is bottom-weighted — but those large trees are the dominant seed source (fecundity scales with stem diameter; Busing & Spies 1995), so their depletion erodes the very recruitment the population depends on. The large-tree deficit in the size structure is therefore both a record of past harvest and a brake on future recovery. This stand-scale viability result complements the landscape projection (Figure 11): logging removes the old-growth canopy that yew recruitment requires, while browse suppresses recruitment where canopy persists — both converging on the same recruitment bottleneck, and both pointing to maintenance of old-growth shade and ungulate management as the decisive conservation levers (§4.3).
...
Figure S5. Stand-level population viability analysis from the Busing & Spies (1995) stage-projection matrix seeded with the observed yew size structure (FAIB + field). (a) Projected stand population relative to today (log scale) under four scenarios: protected old-growth (λ = 1.02), ungulate browse removing 70% of recruitment (λ = 0.99), continued large-tree harvest at 2%/yr (λ = 1.02), and browse + harvest combined (λ = 0.99). (b) Quasi-extinction probability (population falling below 10% of current within 200 yr; 500 stochastic replicates, vital-rate CV = 0.15): negligible under protection, 12% under browse alone, and 42% under browse + harvest. Matrix elasticities are highest in the small/mid size classes, identifying recruitment — not adult survival — as the demographic lever.
