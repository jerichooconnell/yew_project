...
The combined training set therefore comprised 12,559 unique records (1,107 positives and 11,452 negatives), with the GEE-derived negatives duplicated 2× at training time for an effective negative weight of 20,446.
...
To test whether the reported AUC reflects discrimination within ecologically similar stands rather than the trivially easy separation of coastal old-growth from alpine or boreal forest, we evaluated the production model against a same-BEC-zone negative set: 2,811 non-yew locations drawn exclusively from CWH, ICH, and CDF zones (the same zones where positives occur), compared with 3,383 "easy" negatives from spectrally dissimilar zones (SBS, IDF, BWBS, SBPS, PP). AUC declined only marginally between the easy (0.9998) and hard (0.9992) negative sets, but the false positive rate within coastal BEC zones was 17× higher (1.0% vs 0.06%), indicating the model does not purely rely on zone identity. These comparisons use the same training distribution (in-sample) and therefore overestimate absolute performance; their value is directional. A fully held-out hard-negative evaluation would require new GEE embeddings beyond the current dataset scope (`scripts/analysis/hard_negative_eval.py`; results in `results/analysis/hard_negative_eval.json`).
...
#### 2.4.4 Independent map validation

To validate the habitat surface against occurrence data without restricting the test to the 98 rendered study tiles, we sampled the 64-band AlphaEarth embedding directly at each independent occurrence point from Google Earth Engine, classified it with the production XGBoost model, and applied the same VRI suppression the published map uses (point-in-polygon lookup in the VEG_COMP inventory; `scripts/analysis/point_map_validation.py`). This point-based scheme is tile-independent — every presence and absence yields a probability regardless of whether it falls inside a rendered tile — and evaluates both the raw classifier surface (the habitat-suitability model) and the final suppressed surface (the published map). Two occurrence datasets were used: FAIB permanent sample plots in the CWH, ICH, and CDF zones (115 yew-present, 2,796 yew-absent sites), which are genuinely independent of classifier training, and the held-out iNaturalist validation split (127 BC positives, 487 negatives), which is only quasi-independent because its positives, although withheld from training, are drawn from the same iNaturalist pool that supplied the training positives. Together these provide 242 independent presence points — comfortably above the ten-point minimum for a reliable continuous Boyce index (Hirzel et al. 2006), the presence-only metric that the earlier tile-restricted matching (about ten in-tile points) could not support.

On the fully independent FAIB data the raw classifier achieved an AUC-ROC of 0.78 across all stand types, rising to 0.85 in undisturbed forest and 0.87 in old-growth (VRI category 7) — the domain the map is designed to represent — and falling to 0.69 in logged or young stands:

| FAIB stand class (raw classifier) | *n* (presence) | AUC-ROC |
|---|---|---|
| Old-growth (VRI cat 7) | 421 (17) | 0.87 |
| Undisturbed (cat 5 + 7) | 1,626 (64) | 0.85 |
| All stands | 2,911 (115) | 0.78 |
| Logged / young (cat 1–4) | 1,281 (51) | 0.69 |

This gradient is itself the validation signal: the model discriminates yew habitat well where habitat and occurrence coincide (old-growth) and poorly in logged stands, where the few surviving yew are isolated leave-trees whose 10 m spectral signature is dominated by the surrounding clearcut. The held-out iNaturalist set gave a higher raw AUC-ROC of 0.96 (Boyce index 0.99), consistent with its occurrence-weighted, less canopy-obscured sampling, but is the more optimistic estimate given its shared provenance with the training positives.

Applying VRI suppression lowered the AUC-ROC (FAIB 0.68, iNaturalist 0.89) because 51 of the 115 FAIB yew-present sites (44%) lie in logged or young stands (VRI categories 1–4) that the map deliberately assigns near-zero probability; the suppressed surface therefore trades occurrence recall for habitat-suitability ranking, exactly as intended, since it depicts recruiting old-growth *habitat* rather than the location of every surviving stem. The suppression pipeline behaved correctly on non-habitat: all 69 validation points falling in water (VRI category 1) were suppressed to exactly zero, as were logged and alpine points. (The raw-surface Boyce index is the reliable presence-only summary here; the suppressed-surface Boyce is unstable because suppression concentrates most background points at exactly zero.)

These figures are conservative lower bounds on the map's true skill because of positional uncertainty in the occurrence coordinates. FAIB coordinates locate the plot centre, but a fixed-area sample plot spans an ~11–18 m radius and GPS fixes under old-growth canopy carry a further ~5–15 m error; iNaturalist coordinates are more variable still, some recorded only to ~100 m. Relative to the 10 m embedding pixel this is a one- to three-pixel positional error that mislocates edge points into adjacent stands and depresses measured accuracy, so the classifier's discrimination at the true tree location is likely somewhat higher than reported. Full output is in `results/analysis/point_map_validation.json`.
...
| 4 | Logged 40–80 yr | ×0.20 |
| 5 | Forest 80–150 yr | ×0.50 |
| 6 | Alpine / barren | ×0.00 |
| 7 | Old-growth (>150 yr) | ×1.00 |

Categories 2 and 3 (logged < 40 yr) and category 6 (alpine) are suppressed to zero, reflecting the absence of yew in recently cleared or non-forested stands. Category 4 (logged 40–80 yr) receives a ×0.20 factor, acknowledging that occasional yew leave-trees or root-sprouting survivors may persist in older second-growth but that stand structure is unsuitable for recruitment. Category 5 (forest 80–150 yr, maturing second-growth) receives a ×0.50 factor: stands of this age may carry residual yew from the pre-harvest cohort, but canopy closure and altered light conditions suppress recruitment below replacement. Only category 7 (old-growth >150 yr) is treated as fully suitable (×1.00). For habitat *loss* estimation the historical baseline uses the raw model probability uniformly across all forested land — old-growth and logged alike — weighted by the BEC-subzone yew prevalence rate (§2.5.4), so that the partial suppression factors apply only to the *current* estimate. This assumption likely inflates the historical baseline: not all forested land that was subsequently logged would have supported yew under old-growth conditions, so the 154,483 ha figure is best interpreted as an upper bound on original suitable habitat rather than a precise census of former range.
...
where *R*(*t*) is the Busing-matrix recovery fraction of the large-tree cohort (*R*(0) = 0, rising to ~0.9 only by *t* ≈ 105 yr). This reproduces Busing & Spies' conclusion that disturbed stands need centuries to recover the population size and structure characteristic of old-forest stands. For overlapping polygons, the most recent fire year takes precedence.
...
The Yew Big Bud Mite (*Cecidophyopsis psilaspis*) causes bud galls on *Taxus* species, with terminal bud mortality averaging over 20% in infested coastal BC populations, reducing growth rates by ~20% and seed (aril) production by ~25% (TODO find citation). No *T. brevifolia*-specific dose-response curve was found in the academic literature or provincial forest health reports; the 20%/25% multipliers used in the population viability analysis (§3.3) are therefore illustrative sensitivity bounds rather than empirically fitted values and should be interpreted accordingly.
...
| Total | 154,483 | 49,663 | 67.9 | 493,039† | 692 |

†The "Logged Area" column reports total logged *forest* area within each zone (all non-old-growth forested land); the column sums to 493,039 ha but this total is distinct from the area of logged *yew habitat* (~104,820 ha = original − remaining). Zone totals reflect overall forest disturbance, not yew-specific loss.

### 3.2 What drove the decline?

#### 3.2.1 Logging as the primary driver

Industrial clear-cut logging accounts for an estimated 104,820 ha of destroyed yew habitat — approximately 99.4% of all ecological loss documented in this study. This dominance is reflected in the suppression pipeline: the logging filter removes 146,506 ha (68.7%) of raw model predictions, far exceeding the contributions of fire (~700 ha) and all secondary threats combined. The three focal zones exhibit qualitatively different logging-driven decline pathways:
...
|--------|------------------|------------|------|
| Clear-cut logging | 104,820 ha total habitat loss | High (modelled) | Direct habitat destruction |
...
To translate the observed size structure into a viability statement, we seeded the Busing & Spies (1995) stage-projection matrix with the observed (FAIB + field) stage structure and projected it forward under the four ongoing threats to BC yew — industrial logging, wildfire, ungulate browse, and big-bud mite — each represented as an independent, biologically-placed perturbation, including a stochastic quasi-extinction analysis (Figure S5; `scripts/analysis/yew_pva.py`). Logging and wildfire enter as catastrophic stand-level hazards (a burn kills 75% of the stand and leaves 25% in refugia; a clearcut locally extirpates shade-obligate yew with ~2% leave-tree retention and no in-place recovery), whereas browse acts on recruitment and seedling/sapling survival and the mite on growth advancement and aril set. Vital rates in the Busing matrix are calibrated to reproduce λ = 1.02 and q = 1.5 for undisturbed old-growth. We independently verified stage-specific annual survival using the 194 multi-visit FAIB TW trees (292 transitions, mean interval 13.1 yr); the FAIB-fitted mean annual survival (0.994/yr) is slightly lower than the Busing-calibrated value (0.998/yr) and does not alter the qualitative conclusions. FAIB-fitted growth-class advancement rates are slower than the Busing model, likely reflecting that the FAIB sample spans degraded as well as prime old-growth stands; Busing-calibrated growth rates are retained for projections (full comparison in `results/analysis/faib_vital_rates.json`). The baseline annual burn probability was updated from a placeholder (0.002/yr) to the BC fire-history estimate of 0.000794/yr (96,543 ha burned across the 980,000 ha study area over 1900–2024). In intact, protected old-growth the modelled stand is viable (λ = 1.02, quasi-extinction probability ≈ 0 over 200 yr), consistent with Busing & Spies' conclusion that undisturbed stands are stable to slowly growing. Critically, the matrix elasticities are highest for recruitment and growth through the small and mid-size classes (stages 4–5: 0.30, 0.28) and lowest for large-tree survival (stages 6–7: 0.06, 0.02), identifying recruitment, not adult survival, as the demographic lever for stand-level viability. The four threats act on this lever very unevenly. *Logging* is by far the dominant factor: modelled as a stand-clearing hazard at the 1.2%/yr rate implied by the mapped 69.2% decline over ~100 yr, it drives quasi-extinction probability to 74% over 200 yr — a median stand is cleared by ~year 59 and ~90% of unprotected stands are cleared within the window — even though the surviving stems retain viable old-growth vital rates. *Ungulate browse* is the strongest chronic stressor: removing 60–80% of seedlings and saplings in high-density areas (§2.6.4), it drops the modelled growth rate below replacement on its own (λ = 0.99, quasi-extinction probability 12%). *Wildfire*, even at ten times the fitted historical rate, leaves the modelled stand growing (λ_eff = 1.02, quasi-extinction probability 2%), confirming that fire alone is demographically insufficient without logging to remove the old-growth refugia — a threshold analysis (`scripts/analysis/fit_demographic_rates.py`) finds that fire would have to run at ~28× the historical average to force an EN-level (50%) decline unaided. *Big-bud mite* (*Cecidophyopsis psilaspis*) is the mildest factor: suppressing growth advancement (−20%) and aril set (−25%), it slows but does not reverse growth (λ = 1.01, quasi-extinction probability ≈ 0). Acting together in an unprotected stand, the four compound to near-certain loss (λ_eff = 0.98, quasi-extinction probability 97%). Large-tree (bark) removal is deliberately excluded from these forward scenarios: large trees are no longer harvested, and in isolation their removal barely changes λ (1.02) because yew's demography is bottom-weighted. But because fecundity scales with stem diameter (Busing & Spies 1995), those large trees are the dominant seed source (reproductive values 8.5 and 8.2, the highest of any stage), so the 1990s-era large-tree deficit persists as both a record of past harvest and a brake on the recruitment-driven recovery the population depends on. This stand-scale result complements the landscape projection (Figure 11): logging removes the old-growth canopy that yew recruitment requires, while browse suppresses recruitment where canopy persists — both converging on the same recruitment bottleneck, and both pointing to maintenance of old-growth shade and ungulate management as the decisive conservation levers (§4.3).
...
A natural extension that would integrate the two halves of this study — the remote-sensing habitat surface and the stand-level demography (§3.3, Figure S5) — is a spatially explicit integral projection model (IPM) that couples local vital rates to the mapped habitat. IPMs parameterised from repeat airborne LiDAR have recently been used to estimate size-dependent growth, survival, and recruitment of forest trees at landscape scale (e.g. remote-sensing demography from multi-temporal LiDAR; TODO cite), providing a template for propagating the viability analysis here across the full mapped extent and projecting yew abundance under spatially varying logging, browse, and fire pressure rather than at a single representative stand.
...
Criterion A2c evaluates population size reductions estimated or inferred where the reduction or its causes may not have ceased, based on observed decline in habitat area, extent, or quality. Pacific yew reaches reproductive maturity at approximately 80–100 years (Graham 1994); three generations therefore span ~240–300 years, well encompassing the industrial logging era beginning in the 1920s. Our analysis documents a 67.9% observed habitat decline across 9,800 km² of sampled BC range over approximately the past 100 years (the period of intensive industrial logging from the 1920s through the present). This is a partial-window observation, not a 3-generation projection: the IUCN A2c criterion permits using a shorter observed window when the causes of decline are ongoing and inferred to have persisted for at least 1 generation. Because industrial logging began in the 1920s and continues under existing tenure commitments, the 3-generation extrapolation would equal or exceed the observed 67.9% figure. A full 3-generation projection using the Busing & Spies stage-projection matrix seeded with current habitat extent and logging rates would require province-wide coverage beyond the 98 studied tiles; we report the 67.9% as a conservative lower bound on 3-generation decline rather than a final estimate.

| IUCN Criterion | Threshold | This Study | Assessment |
|:---|:---|:---|:---|
| CR A2c | ≥ 80% reduction | 99.1% (CDF); 74.7% (ICH) | CR at subpopulation level |
| EN A2c | ≥ 50% reduction | 67.9% (BC overall) | EN at population level |
| VU A2c | ≥ 30% reduction | Exceeded | — |

A 67.9% estimated habitat decline where the cause is ongoing and not reversible is consistent with the Endangered (EN) threshold (≥50%), recognising that this inference rests on modelled habitat rather than a direct population census. The 99.1% CDF decline and 74.7% ICH decline each individually exceed the Critically Endangered threshold (≥80%) at the subpopulation level.
...
This study presents the first spatially explicit, province-scale assessment of Pacific yew (*Taxus brevifolia*) habitat extent and decline in British Columbia, using 64-dimensional satellite spectral embeddings from the Google AlphaEarth Foundation model classified by an XGBoost ensemble (AUC-ROC 0.996). Across 9,800 km² of sampled CWH, ICH, and CDF forest, we estimate a 67.9% decline from approximately 154,483 ha of original habitat to 49,663 ha remaining — driven overwhelmingly by industrial clear-cut logging, with secondary contributions from wildfire, riparian erosion, and the legacy of taxol bark extraction. Calibrated against 461 deduplicated FAIB permanent sample plot records (conditional mature-stem density: median 10 stems/ha, IQR 4.9–24.7), the remaining 49,663 ha supports an estimated ~497,000 mature individuals. Both the field-measured sample (*n* = 120; binomial *p* ≈ 4 × 10⁻⁹) and the FAIB inventory (*n* = 461 deduplicated stems; *p* ≈ 1.2 × 10⁻⁴³) independently confirm severe depletion of the large-adult cohort (DBH > 30 cm) relative to the de Liocourt stable-population model, consistent with selective harvest of large individuals during the 1989–1993 taxol rush.

The finding that only 11.0% of mapped yew habitat falls within protected-area designations underscores the species' extreme exposure to continued forestry operations. Application of IUCN Red List criteria to the Canadian BC population supports a classification consistent with Endangered (EN A2c), a significant upgrading from the current global Near Threatened listing, with the CDF and ICH subpopulations individually meeting Critically Endangered thresholds. We urge a formal COSEWIC and IUCN reassessment of the species drawing on provincial-scale spatial data, and recommend that Pacific yew habitat be explicitly incorporated into forest stewardship plans across the Coastal Western Hemlock and Interior Cedar–Hemlock zones.
...
Figure S5. Stand-level population viability analysis from the Busing & Spies (1995) stage-projection matrix seeded with the observed yew size structure (FAIB + field), under the four ongoing threats to BC yew as independent perturbations. (a) Median stochastic stand trajectory relative to today (log scale; shaded 10–90th-percentile band shown for the protected and unprotected cases): protected old-growth (λ_eff = 1.02), big-bud mite (λ_eff = 1.01), ungulate browse removing 70% of recruitment and seedling/sapling survival (λ_eff = 0.99), wildfire at 10× the fitted historical burn rate (λ_eff = 1.02), industrial logging as a 1.2%/yr stand-clearing hazard (λ_eff = 1.01 between disturbances; median stand cleared ~year 59), and all four combined (λ_eff = 0.98). (b) Quasi-extinction probability (population falling below 10% of current within 200 yr; 500 stochastic replicates, vital-rate CV = 0.15): 0% protected, 0% mite, 2% wildfire ×10, 12% browse, 74% logging, and 97% all four. Logging is by far the dominant threat and fire alone is demographically insufficient; matrix elasticities are highest in the small/mid size classes, identifying recruitment — not adult survival — as the demographic lever.
