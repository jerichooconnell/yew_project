...
The final suppressed-probability habitat map was validated against two datasets that were independent of both classifier training and tile selection: (1) the held-out iNaturalist validation split (208 BC positives; 6 matched study tiles) and (2) FAIB PSP sites (2,911 sites; 167 matched tiles, 4 with *T. brevifolia*). All 6 iNat validation-set positives within study tiles, and all 4 yew-present FAIB PSP sites within tiles, occurred in VRI logging categories 1–4 (water or logged stands), where the suppression pipeline correctly assigns near-zero probability. No independent yew-present occurrence records within study tiles coincided with undisturbed old-growth (VRI cat 7), preventing a formal sensitivity estimate for the unsuppressed map. The Boyce index — the standard presence-only validation metric — could not be computed reliably with fewer than 10 matched presence points (`scripts/analysis/map_validation_boyce.py`; full output in `results/analysis/map_validation_boyce.json`). This limitation reflects the map's purpose: it depicts old-growth yew *habitat*, not the location of every surviving individual; yew in logged stands (leave-trees) exist but do not constitute habitat in the recruitment sense the map models.
